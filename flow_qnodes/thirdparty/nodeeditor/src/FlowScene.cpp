...
FlowScene::createConnection(Node &nodeIn, PortIndex portIndexIn, Node &nodeOut,
...
void FlowScene::iterateOverNodeDataDependentOrder(
  ...
  auto areNodeInputsVisitedBefore = [&](Node const &node,
    ...
    for (size_t i = 0; i < model.nPorts(PortType::In); ++i) {
      auto connections =
          node.nodeState().connections(PortType::In, QtNodes::PortIndex(i));
...
void FlowScene::save() const {
  QString fileName = QFileDialog::getSaveFileName(
      ...
      tr("Flow Scene Files (*.flow)"), Q_NULLPTR,
...
void FlowScene::load() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      nullptr, tr("Open Flow Scene"), QDir::homePath(),
      tr("Flow Scene Files (*.flow)"), 0,
      QFileDialog::
          DontUseNativeDialog); // 666 Crashing for some reason in Windows, not
                                // using native prevents it.

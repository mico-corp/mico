...
Switch::Switch(QWidget *parent)
    : QAbstractButton(parent), _height(16), _opacity(0.000), _switch(false),
  ...
  connect(this, &QAbstractButton::toggled,
          [&](bool _checked) { animationPlay(_checked); });
}
...
void Switch::paintEvent(QPaintEvent *) {
  ...
  if (isEnabled()) {
    ...
    p.drawRoundedRect(
        QRect(_margin, _margin, width() - 2 * _margin, height() - 2 * _margin),
        8.0, 8.0);
    p.setBrush(_thumb);
    p.setOpacity(1.0);
    p.drawEllipse(QRectF(offset() - (_height / 2), _y - (_height / 2), height(),
                         height()));
  } else {
    p.setBrush(Qt::black);
    p.setOpacity(0.12);
...
void Switch::animationPlay(bool _checked) {
  _switch = _checked;

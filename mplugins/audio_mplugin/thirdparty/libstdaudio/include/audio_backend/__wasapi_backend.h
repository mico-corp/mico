		...
		if (!_running)
		{
			...
			if (!_user_callback.valueless_by_exception())
			{
				_processing_thread = thread{ [this]()
				{
					...
					while (_running)
					{
						if (std::holds_alternative< __wasapi_float_callback_t>(_user_callback)) {
							auto call = std::get< __wasapi_float_callback_t>(_user_callback);
							process(call);
						}else if (std::holds_alternative< __wasapi_int32_callback_t>(_user_callback)) {
							auto call = std::get< __wasapi_int32_callback_t>(_user_callback);
							process(call);
						}else if (std::holds_alternative< __wasapi_int16_callback_t>(_user_callback)) {
							auto call = std::get< __wasapi_int16_callback_t>(_user_callback);
							process(call);
						}

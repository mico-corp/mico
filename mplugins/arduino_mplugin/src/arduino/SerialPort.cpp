...
SerialPort::SerialPort(std::string _port, unsigned int _baudRate)
    : io_(), serial_(io_, _port) {
  serial_.set_option(boost::asio::serial_port_base::baud_rate(_baudRate));
}

bool SerialPort::isOpen() const { return serial_.is_open(); }

void SerialPort::close() { serial_.close(); }

size_t SerialPort::writeString(std::string _s) {
  return boost::asio::write(serial_,
                            boost::asio::buffer(_s.c_str(), _s.size()));
}

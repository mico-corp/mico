// Resource by https://github.com/tobiasThomer/Joypad-Widget
...
JoyPad::JoyPad(QWidget *parent, Qt::WindowFlags f)
    : QWidget(parent, f), m_x(0), m_y(0),
 ...
 * @return
 */
float JoyPad::x() const { return m_x; }

/**
 * @brief JoyPad::y
 * @return
 */
float JoyPad::y() const { return m_y; }
...
void JoyPad::setX(float value) {
  ...
  m_knopBounds.moveCenter(
      QPointF(m_bounds.center().x() + m_x * radius, m_knopBounds.center().y()));
...
void JoyPad::setY(float value) {
  ...
  qreal radius = (m_bounds.width() - m_knopBounds.width()) / 2;
  m_knopBounds.moveCenter(
      QPointF(m_knopBounds.center().x(), m_bounds.center().y() - m_y * radius));
...
void JoyPad::setAlignment(Qt::Alignment f) { m_alignment = f; }
...
void JoyPad::paintEvent(QPaintEvent *event) {
  ...
  // draw background
  QRadialGradient gradient(m_bounds.center(), m_bounds.width() / 2,
                           m_bounds.center());
  ...
  gradient = QRadialGradient(m_knopBounds.center(), m_knopBounds.width() / 2,
                             m_knopBounds.center());
